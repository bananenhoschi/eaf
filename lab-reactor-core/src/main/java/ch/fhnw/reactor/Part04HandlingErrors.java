...
public class Part04HandlingErrors {
    ...
    public Flux<Integer> errorIsTerminal(Flux<String> numbers) {
        return numbers.map(Integer::parseInt).log();
    }
    ...
    public Flux<Integer> handleErrorWithFallback(Flux<String> numbers) {
        return numbers.map(Integer::parseInt).onErrorReturn(NumberFormatException.class, 0).log();
    }
    ...
    public Flux<Integer> handleErrorAndContinue(Flux<String> numbers) {
        return numbers.flatMap(s -> Mono.just(s).map(Integer::parseInt).onErrorReturn(NumberFormatException.class, 0)).log();
    }
